...
@triton.jit
def _fused_append_shared_experts_sigmoid_kernel(
    topk_ids_ptr,
    topk_weights_ptr,
    gate_logits_ptr,
    out_ids_ptr,
    ...
    """Fused kernel: copy topk + sigmoid(bf16 logits -> fp32) + append shared experts."""
    pid = tl.program_id(0)
    ...
    mask_s = offs_s < S
    shared_ids = tl.cast(N_BASE + offs_s, ids.dtype)
    # Load bf16 gate logits, cast to fp32 FIRST, then compute sigmoid in fp32
    raw_logits = tl.load(gate_logits_ptr + pid * S + offs_s, mask=mask_s)
    logits_f32 = raw_logits.to(tl.float32)
    shared_ws = tl.sigmoid(logits_f32)

    tl.store(out_ids_ptr + out_row_ptr + K + offs_s, shared_ids, mask=mask_s)
    tl.store(out_weights_ptr + out_row_ptr + K + offs_s, shared_ws, mask=mask_s)


def fused_append_shared_experts_with_sigmoid(
    topk_ids, topk_weights, gate_logits, num_fused_shared_experts, N=None
):
    """Fused sigmoid + append: load bf16 gate logits -> fp32 sigmoid -> append to topk output.

    This avoids 3 separate kernels (sigmoid, bf16->fp32 cast, append) and ensures
    sigmoid is computed in fp32 precision.
    """
    assert N is not None, "N (shared expert base id) must be provided"
    m, k = topk_ids.shape
    s = int(num_fused_shared_experts)
    ...
    gate_logits_2d = gate_logits
    if gate_logits_2d.ndim == 1:
        gate_logits_2d = gate_logits_2d.unsqueeze(-1)
    if gate_logits_2d.shape[1] < s:
        gate_logits_2d = gate_logits_2d.expand(m, s)
    gate_logits_2d = gate_logits_2d.contiguous()
    ...
    _fused_append_shared_experts_sigmoid_kernel[(m,)](
        topk_ids,
        topk_weights,
        gate_logits_2d,
        out_ids,
